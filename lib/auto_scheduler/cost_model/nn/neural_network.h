 ...
 public:
  NeuralNetworkCostModel() {
    ...
    for (int i = 0; i < nof_samples; i++) {
      ...
      float label = benchmark_w * sample + benchmark_b +
                    std::rand() / float(RAND_MAX) * 0.01;
    ...
    auto w =
        tensorflow::ops::Variable(root, {1, 1}, tensorflow::DataType::DT_FLOAT);
    auto assign_w = tensorflow::ops::Assign(
        root, w,
        tensorflow::ops::RandomNormal(root, {1, 1},
                                      tensorflow::DataType::DT_FLOAT));
    auto b =
        tensorflow::ops::Variable(root, {1, 1}, tensorflow::DataType::DT_FLOAT);
    auto assign_b = tensorflow::ops::Assign(root, b, {{0.0f}});
    auto y_ =
        tensorflow::ops::Add(root, tensorflow::ops::MatMul(root, x, w), b);
    auto loss =
        tensorflow::ops::L2Loss(root, tensorflow::ops::Sub(root, y_, y));
    ...
    auto apply_w = tensorflow::ops::ApplyGradientDescent(root, w, learn_rate,
                                                         {grad_outputs[0]});
    auto apply_b = tensorflow::ops::ApplyGradientDescent(root, b, learn_rate,
                                                         {grad_outputs[1]});
    ...
    for (int epoch = 1; epoch <= 64; epoch++) {
      ...
      for (int i = 0; i < nof_samples; i++) {
        TF_CHECK_OK(
            sess.Run({{x, {{dataset[i].sample}}}, {y, {{dataset[i].label}}}},
                     {w, b, loss, apply_w, apply_b}, &outputs));
      }

      //   LOG(INFO) << "epoch " << epoch << ": w=" <<
      //   outputs[0].matrix<float>()
      //             << " b=" << outputs[1].matrix<float>()
      //             << " loss=" << outputs[2].scalar<float>();
    }
    ...
    //           << t1.tv_sec - t0.tv_sec +
    //                  (t1.tv_nsec - t0.tv_nsec) * 1.0 / 1000000000
    //           << "s";
  }
